...
static int is_ignored(const char* list, const char* filename)
{
	char* next = list;
	...
	while(next != NULL)
	{
		next = strstr(next, filename);
		if(next == NULL)
		{
			return 0;
		}

		if(next == list || isspace(next[-1]))
		{
			next += strlen(filename);
			if(next[0] == '\0' || isspace(next[0]))
			{
				return 1;
			}
		}
	...
	if(S_ISDIR(st.st_mode))
	{
		...
		if(dir != NULL)
		{
			...
			while((iter = readdir(dir)) != NULL)
			{
				...
				if(is_ignored(ignore, iter->d_name))
				{
					printf("ignore %s\n", filename);
					continue;
				}
