...
int data_safe_admin(DataSafeCmd* proxy, int sock, int argc, char* argv[])
{
	...
	if(argc != 4)
	{
		...
		printf("  examples:\n");
		printf("  %s --passwd=12345678 --action=adduser --args=lixianjing\n", argv[0]);
		printf("  %s --passwd=12345678 --action=deluser --args=lixianjing\n", argv[0]);
		printf("  %s --passwd=12345678 --action=changepasswd --args=lixianjing:12345678\n", argv[0]);
		printf("  %s --passwd=12345678 --action=getpolicy   --args=none\n", argv[0]);
		printf("  %s --passwd=12345678 --action=setpolicy   --args=policy.txt\n", argv[0]);
		printf("  %s --passwd=12345678 --action=getusers    --args=none\n", argv[0]);
		printf("  %s --passwd=12345678 --action=getmacaddrs --args=none\n", argv[0]);
		printf("  %s --passwd=12345678 --action=setmacaddrs --args=macaddrs.txt\n", argv[0]);
		printf("=====================================================================\n");
